function [ Freq, Vmag_dB, Vpha_deg, Hmag_dB, Hpha_deg, constants, Date ] = HowlandReadAP( PATH, FNAME  )
...
if( isempty(path) || isempty(fname) )
    ...
    uiText = 'Select Howland AP data file ';
    [fname, path] = uigetfile('*AP*.txt', uiText);
end
...
TestType = strtok(remainder,':');

%------ Verify test type is 'discrete' for antenna pattern (AP) files ----
if( ~isequal(TestType,'Discrete') )
    disp(sprintf('Warning:: Antenna Pattern (AP) file is not of type DISCRETE!!'));
end
%-------------------------------------------------------------------------
...
teststr = 'HPOL Peak Gain Factor';
% Keep reading lines until teststr is found
while (isempty(findstr(teststr,txtline)))
    txtline=fgets(fid);
end
[predicate, remainder] = strtok(txtline,'=');
calFact_Hpol = strtok(remainder,' =');

teststr = 'VPOL Peak Gain Factor';
% Keep reading lines until teststr is found
while (isempty(findstr(teststr,txtline)))
    txtline=fgets(fid);
end
[predicate, remainder] = strtok(txtline,'=');
calFact_Vpol = strtok(remainder,' =');
...
if( theta_stop < 180 )
    theta_stop = 180;
    %nThetaAddPts = (180 - theta_stop) / theta_incr;
end
...
noiseFloor = -45.0;
Vmag_dB = noiseFloor*ones(nTheta,nPhi+1);
Vpha_deg = noiseFloor*ones(nTheta,nPhi+1);
Hmag_dB = noiseFloor*ones(nTheta,nPhi+1);
Hpha_deg = noiseFloor*ones(nTheta,nPhi+1);
while ~feof(fid) 
    ...
    k = k + 1;
    theta = 1 + ( floor(line(1)) - theta_start ) / theta_incr;
    phi = 1 + ( floor(line(2)) - phi_start ) / phi_incr;
