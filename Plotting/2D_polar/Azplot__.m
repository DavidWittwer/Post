function hpol = Azplot__(theta,rho,line_style,params)
%AZplot tested and verified for correct plot orientation on 24Aug03
%to work properly, data must by output using these NSI parameters
%Vcut, Th/Phi data coord system, Phi Span 360, Phi Center 0, # of Phi Pts 361
%Theta Span 90 Theta Center 90 # of Theta Pts 1
...
if ~hold_state    
    ...
    for i=click:click:rngdisp
        ...
        hhh = plot(xunit*i,yunit*i,ls,'color',tc,'linewidth',.25,...
            'handlevisibility','off');
        if i < rngdisp
            text(i*c88,i*s88, ...
                ticktext,'verticalalignment','bottom',...
                'handlevisibility','off','fontsize',12,'fontweight','demi')
        else
            text(i*c88,i*s88, ...
    ...
    th = (0:18)*2*pi/36;
        ...
        % plot spokes at 30 degree intervals
    %th = (0:3:18)*2*pi/36
    
    cst = cos(th); snt = sin(th);
    cs = [-cst; +cst];
    sn = [-snt; +snt];
    plot(rngdisp*cs,rngdisp*sn,ls,'color',tc,'linewidth',.25,...
        'handlevisibility','off')
    
    % label spokes in 30 degree intervals
    rt = 1.125*rngdisp;
%    for i = 1:3:19 %used for 10 degree increments in th
for i = 1:3:19 %used for 10 degree increments in th, changed by SK 1jul03
   %for i=1:7 %used for 30 increments
        text(rt*cst(i),rt*snt(i),[int2str(0+(i-1)*10),'^o'],...
            'horizontalalignment','center',...
            'handlevisibility','off','fontsize',14); 
    end
    if fullplot
        for i = 4:3:16
            text(-rt*cst(i),-rt*snt(i),[int2str(-180 + (i-1)*10),'^o'],...
                'horizontalalignment','center',...
                'handlevisibility','off','fontsize',14); 
        end
%         for i = 9:3:15
...
%SK:  changed (-) theta to pos, on 1jul03
%SK: remove pi/2 rotation, to properly orient data w/ NF Chamber coord
%system, THEN change cos/sin functions to (-) (thus matching plot label
%layout)

xx = (rho+rngdisp-rmax).*(-cos(theta));
yy = (rho+rngdisp-rmax).*(-sin(theta));

% plot data on top of grid
if strcmp(line_style,'auto')
    q = plot(xx,yy,'linewidth',2);
else
    q = plot(xx,yy,line_style,'linewidth',2.5);
end
if nargout > 0
    hpol = q;
end
